...
- **Bypass login for the key-guarded paths**: `/v1/`, `/models`,
  `/upstream/`, `/comfyui/`, and `/metrics`, `/unload`, `/running` if
  anything external calls them. llama-swap checks the key itself.
