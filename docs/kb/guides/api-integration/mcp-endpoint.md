...
config_keys: [apiKeys, auth.trustedHeader]
updated: 2026-08-27
---
...
The endpoint counts as part of the web UI. With `auth.trustedHeader` set it
accepts only requests the proxy has vouched for, and API keys no longer
open it, because `config__get_config` exposes the running configuration.
External MCP clients then have to go through the proxy's login.
